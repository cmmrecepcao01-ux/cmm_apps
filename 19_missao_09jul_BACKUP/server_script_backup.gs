...
function doPost(e) {
  try {
    ...
    if (action === "save_layout") {
      ...
      // Escreve cabeçalhos do layout (Inclui tripulação)
      layoutSheet.appendRow(["OPM", "VTR", "X", "Y", "Rotation", "Motorista", "Encarregado"]);
    ...
    if (action === "update_checklist") {
      ...
      for (var r = 1; r < values.length; r++) {
        if (values[r][opmColIdx] === opm && values[r][vtrColIdx] === vtr) {
          foundRowIdx = r + 1;
          break;
        }
      }
      ...
      if (foundRowIdx !== -1) {
        for (var col = 1; col <= rowData.length; col++) {
          sheet.getCell(foundRowIdx, col).setValue(rowData[col - 1]);
        }
      } else {
